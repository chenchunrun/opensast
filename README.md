...
**核心心智模型：** OpenSAST 是一条 `scan → triage → fix → verify` 流水线，**规则只是输入，LLM/Agent 才是主分析器**。完整使用指南（心智模型 / 按场景决策 / baseline & CI 运营 / 踩坑提示）见 [`docs/USAGE.md`](docs/USAGE.md)。
...
# ② 分诊：LLM 判 TP/FP，误报进 baseline 静默
...
- **开发者本地**：`quick --changed-only` → `/sast-fix` 高危项
- **仓库级评审**：`standard` → `/sast-triage --bulk` → baseline/fix
- **新项目首次接入**：`standard` → `/sast-triage` **建 baseline** → fix
- **高信任审计**：`deep` → triage → fix → `--fail-on high` 门禁

> ⚠️ baseline（`.claude/sast/baseline.json`）被 gitignore 排除，是 **per-machine** 的——CI 拿不到本地 baseline。CI 变绿需靠 `config.yml` 的 `targets.exclude` 排除固有噪声（如 demo/fixture），详见 [`docs/USAGE.md`](docs/USAGE.md) §4。

底层工具（skill 内部调用，一般无需直用）：

```bash
# 分类输出
python3 .claude/skills/sast-scan/tools/triage_findings.py \
  --findings .claude/sast/results/findings.json --focus all --output markdown
...
python3 .claude/skills/sast-scan/tools/baseline_manager.py suppress \
  --fingerprint <fingerprint> --reason "false positive with framework guard"
